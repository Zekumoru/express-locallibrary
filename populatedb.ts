...
async function authorCreate(
  ...
  first_name: string,
  family_name: string,
  d_birth: string | boolean,
  d_death: string | boolean
) {
  const authordetail: {
    first_name: string;
    family_name: string;
  ...
  if (d_birth != false) authordetail.date_of_birth = d_birth as string;
  if (d_death != false) authordetail.date_of_death = d_death as string;
